/**
 * Script para testar o processamento de replies com odd = 0 (não conseguiu pegar)
...
const testBetKey = '670237902_456';
const testBetData = {
  jogo: "Teste Zero vs Não Conseguiu",
  placar: "2-1",
  mercado: "Ambas Marcam",
  linha_da_aposta: "Sim",
  odd_tipster: "2.10",
  grupo: "Teste Local Zero",
...
// Simular um reply do Telegram com odd = 0 (não conseguiu pegar)
const mockReplyUpdate = {
  ...
  message: {
    ...
    text: "0", // Usuário não conseguiu pegar a aposta
    reply_to_message: {
      message_id: 456, // Deve corresponder ao ID na chave do cache
      from: {
        id: 7487941746,
        is_bot: true,
...
console.log('🧪 Enviando teste de reply com odd = 0 para o webhook local...');
console.log('📤 URL:', `http://${options.hostname}:${options.port}${options.path}`);
console.log('📦 Payload (odd = 0):', JSON.stringify(mockReplyUpdate, null, 2));
console.log('');
    ...
    setTimeout(() => {
      ...
      if (fs.existsSync(cacheFile)) {
        ...
        if (updatedCache[testBetKey]) {
          ...
          console.log('✅ Aposta foi removida do cache - processada como NÃO PEGA!');
        }
      } else {
      ...
      console.log('📝 A aposta deve ter sido salva no Google Sheets como "NÃO PEGA".');
    }, 1000);
  });
});
